...
uint64_t* secondContext = (uint64_t*) 0;
...
uint64_t doublemipster(uint64_t* contextList);
uint64_t doublehypster(uint64_t* contextList);
...
uint64_t doublemipster(uint64_t* contextList) {
  ...
  toContext = (uint64_t*) *(contextList+index);
    ...
    if (getParent(fromContext) != MY_CONTEXT) {
      ...
      if (getException(fromContext) == EXCEPTION_PAGEFAULT) {
        ...
        mapPage(fromContext, getFaultingPage(fromContext), (uint64_t) palloc());
				...
				printInteger(getExitCode(fromContext));
				print((uint64_t*) " and ");
        ...
        if(*(contextList+index) == 0)
          index = 0;
				fromContext = (uint64_t*) *(contextList+index);
			}
...
uint64_t selfie_run(uint64_t machine) {
  uint64_t exitCode;
  uint64_t* contextList; //  null terminated list
  ...
  contextList = malloc(12);
  *contextList = (uint64_t) currentContext;
  *(contextList+1) = (uint64_t) secondContext;
  *(contextList+2) = 0;
  ...
  if (machine == MIPSTER)
    ...
    exitCode = doublemipster(contextList);
	else if (machine == DOUBLEHYPSTER)
    if (isBootLevelZero())
      // no hypster on boot level zero
      exitCode = doublemipster(contextList);
    else
      exitCode = doublehypster(contextList);
