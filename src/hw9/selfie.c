...
uint64_t* writeLockContext = (uint64_t*) 0;
...
void setProgramBreakForAll(uint64_t* context, uint64_t newPB);
...
// hattingerse
uint64_t* secondContext;
uint64_t* thirdContext;

uint64_t debug_locks  = 0;
uint64_t debug_fork = 0;
uint64_t debug_forked = 0;
uint64_t debug_kill = 0;
uint64_t debug_wait = 0;
uint64_t debug_thread = 1;
uint64_t debug_CAS = 1;
...
uint64_t SYSCALL_LOCK = 4091;
uint64_t SYSCALL_UNLOCK = 4092;
uint64_t SYSCALL_FORK = 4093;
uint64_t SYSCALL_WAIT = 4094;
uint64_t SYSCALL_KILL = 4095;
uint64_t SYSCALL_THREAD = 4096;
uint64_t SYSCALL_CAS = 4097;

uint64_t STATE_READY = 1;
uint64_t STATE_EXITED = 2;
uint64_t STATE_WAITING = 3;
...
uint64_t* addContextCL(uint64_t* context, uint64_t* in);
uint64_t* removeContextCL(uint64_t* context, uint64_t* in);
uint64_t* findElementCL(uint64_t* context, uint64_t* in);

uint64_t* copyContextOld(uint64_t* context);
uint64_t* copyContext(uint64_t* context);
uint64_t* copyContextToThread(uint64_t* context);
uint64_t  existsExecutableContext();
uint64_t* getNextExecutableContext(uint64_t* context);
uint64_t* getNextExecutableContext2(uint64_t* context);
uint64_t* getChildList(uint64_t* context);
void handleExitSyscall(uint64_t* context);
void handleWaitingState(uint64_t* context);
uint64_t killContext(uint64_t pid);
void mapOrLinkPage(uint64_t* context, uint64_t page);

void printOverviewCL(uint64_t* cl);
void printDetailsCL(uint64_t* cl);
void printContext(uint64_t* context);

// execContexts List Element
// +----+----------------+
// |  0 | nextContext    | pointer to next context
// |  1 | context        | context to execute
// +----+----------------+

uint64_t* getNextCL(uint64_t* contextListElement)    { return (uint64_t*) *contextListElement; }
uint64_t* getContextCL(uint64_t* contextListElement)          { return (uint64_t*) *(contextListElement + 1); }

void setNextCL(uint64_t* contextListElement, uint64_t* next)     { *contextListElement        = (uint64_t) next; }
void setContextCL(uint64_t* contextListElement, uint64_t* context)      { *(contextListElement + 1)  = (uint64_t) context; }
...
uint64_t State(uint64_t* context)          { return (uint64_t) (context + 17); }
uint64_t ForkParent(uint64_t* context)     { return (uint64_t) (context + 18); }
uint64_t Pid(uint64_t* context)            { return (uint64_t) (context + 19); }
uint64_t ThreadParent(uint64_t* context)   { return (uint64_t) (context + 20); }
...
uint64_t  getState(uint64_t* context)          { return             *(context + 17); }
uint64_t* getForkParent(uint64_t* context)     { return (uint64_t*) *(context + 18); }
uint64_t  getPid(uint64_t* context)            { return             *(context + 19); }
uint64_t* getThreadParent(uint64_t* context)   { return (uint64_t*) *(context + 20); }
...
void setState(uint64_t* context, uint64_t state)                 { *(context + 17) = state; }
void setForkParent(uint64_t* context, uint64_t* forkParent)      { *(context + 18) = (uint64_t) forkParent; }
void setPid(uint64_t* context, uint64_t pid)                     { *(context + 19) = pid; }
void setThreadParent(uint64_t* context, uint64_t* threadParent)  { *(context + 20) = (uint64_t) threadParent; }
...
uint64_t* contextList; //  null terminated list of running contexts
uint64_t* execContexts = (uint64_t*) 0;
...
uint64_t multimipster();
uint64_t multihypster();
...
void implementExit(uint64_t* context) {
  ...
  if(getThreadParent(context) == (uint64_t*) 0) {
    print(selfieName);
    print((uint64_t*) ": ");
...
void emitLock() {
  createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "lock", 0, PROCEDURE, UINT64_T, 0, binaryLength);
...
void implementLock(uint64_t* context) {
  ...
  //size  = *(getRegs(context)+REG_A2);
  if(writeLockContext == (uint64_t*) 0) {
    writeLockContext = context;

    if (debug_locks) {
      print(selfieName);
      print((uint64_t*) ": LOCK set on context ");
      printInteger(context);
      println();
    }
  } else if (writeLockContext != context) {
    setPC(context, getPC(context) - INSTRUCTIONSIZE);
  }
}

void emitUnlock() {
  createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "unlock", 0, PROCEDURE, UINT64_T, 0, binaryLength);
...
void implementUnlock(uint64_t* context) {
  ...
  if(writeLockContext == context) {
    writeLockContext = (uint64_t*) 0;

    if (debug_locks) {
      print(selfieName);
      print((uint64_t*) ": UNLOCK on context ");
      printInteger(context);
      println();
    }
  }
...
void implementFork(uint64_t* context) {
  ...
  *(getRegs(newContext) + REG_V0) = 0;

  *(getRegs(context) + REG_V0)  = getPid(newContext);

  setForkParent(newContext, context);

  execContexts = addContextCL(newContext, execContexts);

  debug_forked = 1;
  if (debug_fork) {
    println();
    print(selfieName);
    print((uint64_t*) ": Fork on context ");
    print((uint64_t*) getName(context)); println();
  }
}
...
void emitKill() {
  ...
  emitIFormat(OP_LD, REG_SP, REG_A0, 0); // pid of child
  emitIFormat(OP_DADDIU, REG_SP, REG_SP, REGISTERSIZE);
...
void implementKill(uint64_t* context) {
  ...
  if (debug_kill) {
    println();
    print((uint64_t*) "kill on pid ");
    printInteger(childPid); println();
  }

  childPid = *(getRegs(context) + REG_A0);

  *(getRegs(context) + REG_V0) = killContext(childPid);
...
void emitWait() {
  ...
  emitIFormat(OP_LD, REG_SP, REG_A0, 0); // address
  emitIFormat(OP_DADDIU, REG_SP, REG_SP, REGISTERSIZE);
...
void implementWait(uint64_t* context) {
  ...
  setState(context, STATE_WAITING);

  if (debug_wait) {
    println();
    print((uint64_t*) "wait on ");
    print((uint64_t*) getName(context)); println();
  }
}

void emitThread() {
  createSymbolTableEntry(LIBRARY_TABLE, (uint64_t*) "thread", 0, PROCEDURE, UINT64_T, 0, binaryLength);
...
void implementThread(uint64_t* context) {
  ...
  if (debug_thread) {
    println();
    print((uint64_t*) "Thread() on ");
    print((uint64_t*) getName(context)); println();
  }

  newThread = copyContextToThread(context);

  execContexts = addContextCL(newThread, execContexts);

  printOverviewCL(execContexts);
}
...
void emitCAS() {
  ...
  //pointer
  emitIFormat(OP_LD, REG_SP, REG_A2, 0);
  emitIFormat(OP_DADDIU, REG_SP, REG_SP, REGISTERSIZE);

  //old
  emitIFormat(OP_LD, REG_SP, REG_A1, 0);
  emitIFormat(OP_DADDIU, REG_SP, REG_SP, REGISTERSIZE);

  //new
  emitIFormat(OP_LD, REG_SP, REG_A0, 0);
  emitIFormat(OP_DADDIU, REG_SP, REG_SP, REGISTERSIZE);

  emitIFormat(OP_DADDIU, REG_ZR, REG_V0, SYSCALL_CAS);
  emitRFormat(OP_SPECIAL, 0, 0, 0, FCT_SYSCALL);

  emitRFormat(OP_SPECIAL, REG_RA, 0, 0, FCT_JR);
}

void implementCAS(uint64_t* context) {
  uint64_t new;
  uint64_t old;
  uint64_t* vaddr;
  uint64_t addressvalue;

  new = *(getRegs(context) + REG_A0);
  old = *(getRegs(context) + REG_A1);
  vaddr = (uint64_t*) *(getRegs(context) + REG_A2);
  addressvalue = loadVirtualMemory(getPT(context), vaddr);

  if (addressvalue == old) {
    storeVirtualMemory(getPT(context), vaddr, new);
  ...
  if (debug_CAS) {
    println();
    print((uint64_t*) "compare_and_swap() from Context ");
    print((uint64_t*) getName(context));
    print((uint64_t*) " old:");printInteger(old);
    print((uint64_t*) " new:");printInteger(new);
    print((uint64_t*) " addressvalue:");printInteger(addressvalue);
    //print((uint64_t*) "address : ");printInteger(vaddr);
  }
...
uint64_t implementMalloc(uint64_t* context) {
  ...
  if (size > stackptr - bump) {
    ...
    *(getRegs(context)+REG_V0) = bump;
    ...
    setProgramBreakForAll(context, bump + size);
...
void setProgramBreakForAll(uint64_t* context ,uint64_t newPB) {
    ...
    contextList = (uint64_t*) 0;
    tempElement = execContexts;

    while(tempElement != (uint64_t*) 0) {
      if (getPid(getContextCL(tempElement)) == getPid(context)) {
        setProgramBreak(getContextCL(tempElement), newPB);
      }

      tempElement = getNextCL(tempElement);
    }
}
...
void implementSwitch() {
  ...
  //println();print((uint64_t*) "AAA: before implementSwitch doswitch");println();
  doSwitch(cacheContext((uint64_t*) *(registers+REG_A0)), *(registers+REG_A1));
  //println();print((uint64_t*) "BBB: after implementSwitch doswitch");println();
}
...
uint64_t* hypster_switch(uint64_t* toContext, uint64_t timeout) {
  // this procedure is only executed at boot level zero
  //println();print((uint64_t*) "AAA: before bootlevel zero hypster switch = mipsterswitch");println();
  return mipster_switch(toContext, timeout);
  //println();print((uint64_t*) "BBB: after bootlevel zero hypster switch = mipsterswitch");println();
}
...
void fct_syscall() {
  ...
  if (interpret) {
    ...
    if (*(registers+REG_V0) == SYSCALL_SWITCH) {
      implementSwitch();
      //println();print((uint64_t*) "BBB: after syscall switch");println();
    } else {
...
void execute() {
  ...
  //if(debug_forked == 1){println();print((uint64_t*) "AAA: execute start");println();}
  ...
  //if(debug_forked == 1){println();print((uint64_t*) "AAA: execute end");println();}
}
...
uint64_t* allocateContext(uint64_t* parent, uint64_t* vctxt, uint64_t* in) {
  ...
  if (freeContexts == (uint64_t*) 0)
    context = smalloc(8 * SIZEOFUINT64STAR + 13 * SIZEOFUINT64);
  else {
  ...
  setForkParent(context, (uint64_t*) 0);
  setThreadParent(context, (uint64_t*) 0);

  setState(context, STATE_READY);

  maxpid = maxpid + 1;
  setPid(context, maxpid);
...
uint64_t* copyContextOld(uint64_t* context) {
  ...
  newContext = createContext(MY_CONTEXT, 0);

  setPC(newContext, getPC(context));
  setLoReg(newContext, getLoReg(context));
  setHiReg(newContext, getHiReg(context));
  setLoPage(newContext, getLoPage(context));
  setMePage(newContext, getMePage(context));
  setHiPage(newContext, getHiPage(context));
  setProgramBreak(newContext, getProgramBreak(context));
  setException(newContext, getException(context));
  setFaultingPage(newContext, getFaultingPage(context));
  setExitCode(newContext, getExitCode(context));
  setParent(newContext, getParent(context));
  setVirtualContext(newContext, getVirtualContext(context));
  setName(newContext, (uint64_t*) "Copied Context");

  r = 0;
  regs = getRegs(context);
  newRegs = getRegs(newContext);
  ...
  setPT(newContext, zalloc(VIRTUALMEMORYSIZE / PAGESIZE * SIZEOFUINT64));
  loPage = getLoPage(newContext);
  mePage = getMePage(newContext);
  hiPage = getHiPage(newContext);
  page = loPage;

  while (page <= mePage) {
    frame = *(getPT(context)+page);

    if (frame != 0) {
      mapPage(newContext, page, (uint64_t) palloc());

      offset = 0;
      while (offset < PAGESIZE / SIZEOFUINT64) {
        vaddr = page * PAGESIZE + offset * SIZEOFUINT64;

        storeVirtualMemory(getPT(newContext), vaddr, loadVirtualMemory(getPT(context), vaddr));
...
uint64_t* copyContext(uint64_t* context) {
  ...
  newContext = createContext(getParent(context), 0);
  ...
  setFaultingPage(newContext, getFaultingPage(context));
  setExitCode(newContext, getExitCode(context));
  setState(newContext, getState(context));
  setName(newContext, (uint64_t*) "Copied Context");
  setForkParent(newContext, getForkParent(context));
  ...
  setPT(newContext, zalloc(VIRTUALMEMORYSIZE / PAGESIZE * SIZEOFUINT64));
  loPage = getLoPage(context);
  mePage = getMePage(context);
  hiPage = getHiPage(context);

  page = 0;
  while(page <= mePage) {
    if(isPageMapped(getPT(context), page)) {

          mapPage(newContext, page, (uint64_t) palloc());
          offset = page * PAGESIZE;
          while(offset < (page+1)*PAGESIZE) {
            storeVirtualMemory(getPT(newContext), offset, loadVirtualMemory(getPT(context), offset));
            offset = offset + 8;
            }
      }
      page = page + 1;
    ...
    page = getPageOfVirtualAddress(VIRTUALMEMORYSIZE - REGISTERSIZE);
    while(page >= hiPage){
      if(isPageMapped(getPT(context), page)) {
          mapPage(newContext, page, (uint64_t) palloc());
          offset = page * PAGESIZE;
...
uint64_t* copyContextToThread(uint64_t* context) {
  ...
  newThread = createContext(getParent(context), 0);

  setPC(newThread, getPC(context));
  setLoReg(newThread, getLoReg(context));
  setHiReg(newThread, getHiReg(context));
  setProgramBreak(newThread, getProgramBreak(context));
  setException(newThread, getException(context));
  setFaultingPage(newThread, getFaultingPage(context));
  setExitCode(newThread, getExitCode(context));
  setState(newThread, getState(context));
  setName(newThread, (uint64_t*) "THREAD");
  setPid(newThread, getPid(context));
  // dont! setForkParent(newThread, getForkParent(context));

  r = 0;
  regs = getRegs(context);
  newRegs = getRegs(newThread);
  ...
  setPT(newThread, zalloc(VIRTUALMEMORYSIZE / PAGESIZE * SIZEOFUINT64));
  loPage = getLoPage(context);
  mePage = getMePage(context);
  hiPage = getHiPage(context);

  //shallow copy non-stack memory
  page = 0;
  while(page <= mePage) {
    if(isPageMapped(getPT(context), page)) {
      mapPage(newThread, page, *(getPT(context) + page));
    }
    page = page + 1;
  }
  ...
  while(page >= hiPage) {
    if(isPageMapped(getPT(context), page)) {
        mapPage(newThread, page, (uint64_t) palloc());

        virtualAddress = page * PAGESIZE;
        while(virtualAddress < (page + 1) * PAGESIZE) {
          storeVirtualMemory(getPT(newThread), virtualAddress, loadVirtualMemory(getPT(context), virtualAddress));

          virtualAddress = virtualAddress + 8;
        }
    }
    page = page - 1;
  }

  //setLoPage(newThread, getLoPage(context));
  //setHiPage(newThread, getHiPage(context));
  //setMePage(newThread, getMePage(context));

  setThreadParent(newThread, context);
...
void printContext(uint64_t* context) {
  ...
  println();
  print((uint64_t*) "  PRINT CONTEXT   "); println();
  print((uint64_t*) "  pointer:        "); printInteger((uint64_t) context); println();
  print((uint64_t*) "  name:           "); print(getName(context)); println();
  print((uint64_t*) "  pc:             "); printInteger(getPC(context)); println();
  print((uint64_t*) "  nextContextptr: "); printInteger((uint64_t) getNextContext(context)); println();
  print((uint64_t*) "  prevContextptr: "); printInteger((uint64_t) getPrevContext(context)); println();
  print((uint64_t*) "  pagetableptr:   "); printInteger((uint64_t) getPT(context)); println();
  print((uint64_t*) "  loPage:         "); printInteger(getLoPage(context)); println();
  print((uint64_t*) "  mePage:         "); printInteger(getMePage(context)); println();
  print((uint64_t*) "  hiPage:         "); printInteger(getHiPage(context)); println();
  print((uint64_t*) "  programbreak:   "); printInteger(getProgramBreak(context)); println();
  print((uint64_t*) "  exception:      "); printInteger(getException(context)); println();
  print((uint64_t*) "  faultingpage:   "); printInteger(getFaultingPage(context)); println();
  print((uint64_t*) "  exitcode:       "); printInteger(getExitCode(context)); println();
  print((uint64_t*) "  parentptr:      "); printInteger((uint64_t) getParent(context)); println();
  print((uint64_t*) "  vctxtptr:       "); printInteger((uint64_t) getVirtualContext(context)); println();
  print((uint64_t*) "  regptr:         "); printInteger((uint64_t) regs); println();

  println();print((uint64_t*) "  REGISTER" ); println();
  ...
  print((uint64_t*) "    |" );
  while (r < NUMBEROFREGISTERS) {
    print(*(REGISTERS + r)); print((uint64_t*) " " );
    printInteger(*(regs + r));print((uint64_t*) "|" );
    if(r == 10) {println();print((uint64_t*) "    |" );}
    if(r == 23) {println();print((uint64_t*) "    |" );}
    r = r + 1;
  }
...
void mapOrLinkPage(uint64_t* context, uint64_t page) {
  uint64_t* threadParent;

  threadParent = getThreadParent(context);

  if (threadParent == (uint64_t*) 0)
    mapPage(context, page, (uint64_t) palloc());
  else {
    if (page > getPageOfVirtualAddress(getProgramBreak(context))) {
      mapPage(context, page, (uint64_t) palloc());
    } else {
      if (isPageMapped(getPT(threadParent), page) == 0)
        mapOrLinkPage(threadParent, page);

      mapPage(context, page, getFrameForPage(getPT(threadParent), page));
    }
  }
}

uint64_t* addContextCL(uint64_t* context, uint64_t* in) {
  uint64_t* newElementCL;
  newElementCL = malloc(2 * SIZEOFUINT64);
  setNextCL(newElementCL, in);
  setContextCL(newElementCL, context);
  return newElementCL;
}

uint64_t* removeContextCL(uint64_t* context, uint64_t* in) {
  uint64_t* tempElement;
  uint64_t* previousElement;
  uint64_t* nextElement;

  previousElement = (uint64_t*) 0;
  tempElement = in;
  nextElement = (uint64_t*) 0;

  while(tempElement != (uint64_t*) 0) {
    nextElement = getNextCL(tempElement);

    if (getContextCL(tempElement) == context) {
      if(previousElement == (uint64_t*) 0)
        in = nextElement;
      else if (nextElement  == (uint64_t*) 0)
        setNextCL(previousElement, (uint64_t*) 0);
    }
...
uint64_t* getChildList(uint64_t* context) {
  ...
  childList = (uint64_t*) 0;
  tempElement = execContexts;

  while(tempElement != (uint64_t*) 0) {
    if (getForkParent(getContextCL(tempElement)) == context) {
      childList = addContextCL(getContextCL(tempElement), childList);
    }
    tempElement = getNextCL(tempElement);
  }
  return childList;
...
// uint64_t* getThreadList() {
//   uint64_t* tempElement;
//   uint64_t* threadList;
//
//   threadList = (uint64_t*) 0;
//   tempElement = execContexts;
//
//   while(tempElement != (uint64_t*) 0) {
//     if (getThreadParent(getContextCL(tempElement)) != (uint64_t*) 0) {
//       threadList = addContextCL(getContextCL(tempElement), threadList);
//     }
//     tempElement = getNextCL(tempElement);
//   }
//   return threadList;
// }
//
// void printThreadList(uint64_t* cl) {
//   uint64_t* tempElement;
//
//   tempElement = cl;
//
...
void printOverviewCL(uint64_t* cl) {
  ...
  tempElement = cl;
  println(); print((uint64_t*) "ContextList" ); println();
  print((uint64_t*) "# | contextPointer | state | pid | thread(parent) | name" ); println();

  i = 1;
  while (tempElement != (uint64_t*) 0) {
    context = getContextCL(tempElement);
    childList = getChildList(getContextCL(tempElement));
    print((uint64_t*) "#" ); printInteger(i);
    print((uint64_t*) "  " ); printInteger((uint64_t) context);
    print((uint64_t*) "  " ); printInteger(getState(context));
    print((uint64_t*) "  " ); printInteger(getPid(context));
    print((uint64_t*) "  " ); printInteger((uint64_t) getThreadParent(context));
    print((uint64_t*) "  " ); print(getName(context));println();
    // print((uint64_t*) "   children: " );
    //
    // while (childList != (uint64_t*) 0) {
    //   printInteger(getContextCL(childList));print((uint64_t*) " ");
    //   //printOverviewCL(childList);
    //   childList = getNextCL(childList);
    // }
    // println();
...
void printDetailsCL(uint64_t* cl) {
  ...
  i = 1;
  println(); print((uint64_t*) "Context List Detailed Elements" ); println();
  while (cl != (uint64_t*) 0) {
    context = getContextCL(cl);
    println();
    print((uint64_t*) "#" ); printInteger(i); println();
    print((uint64_t*) "ContextListElementPtr:  "); printInteger((uint64_t) cl); println();
    print((uint64_t*) "nextElementPtr:         "); printInteger((uint64_t) getNextCL(cl)); println();
    print((uint64_t*) "contextPtr:             "); printInteger((uint64_t) context); println();
...
uint64_t existsExecutableContext() {
  ...
  while (cl != (uint64_t*) 0) {
    if (getParent(getContextCL(cl)) == MY_CONTEXT)
      if (getState(getContextCL(cl)) != STATE_EXITED)
        return 1;
...
uint64_t killContext(uint64_t pid) {
  uint64_t* foundContext;
  uint64_t* tempCL;

  foundContext = (uint64_t*) 0;
  tempCL = execContexts;

  while (tempCL != (uint64_t*) 0) {

    if (getPid(getContextCL(tempCL)) == pid) {
      foundContext = getContextCL(tempCL);
    }

    tempCL = getNextCL(tempCL);
  }

  if (foundContext != (uint64_t*) 0) {

    implementExit(foundContext);
    setState(foundContext, STATE_EXITED);
...
void handleWaitingState(uint64_t* context) {
  ...
  println();print((uint64_t*) "context ");
  print((uint64_t*) getName(context));print((uint64_t*) " is waiting "); println();
  //printOverviewCL(execContexts);
  ...
  while (childList != (uint64_t*) 0) {
    childContext = getContextCL(childList);
    if (getState(childContext) == STATE_EXITED) {

      setState(context, STATE_READY);

      *(getRegs(context)+REG_V0) = getPid(childContext);

      setExitCode(childContext, 42);
      storeVirtualMemory(getPT(context), *(getRegs(context)+REG_A0), getExitCode(childContext));

      //execContexts = removeContextCL(childContext, execContexts);

      if (debug_wait) {
        print((uint64_t*) "parent with pid ");
        printInteger(getPid(context));
        print((uint64_t*) " finished waiting on child with pid ");
        printInteger(getPid(childContext));
        println();
      }
    }
    //setState(context, STATE_READY);
...
void handleExitSyscall(uint64_t* context) {
  uint64_t* tempExecContextCL;
  setState(context, STATE_EXITED);

  // remove thread from executable list if exited
  if(getThreadParent(context) != (uint64_t*) 0) {
    //println();print(getName(context));print((uint64_t*) " dying");println();

    execContexts = removeContextCL(context, execContexts);
    usedContexts = deleteContext(context, usedContexts);
  } else {
    //remove all threads of a process if exited
    tempExecContextCL = execContexts;
    while(tempExecContextCL != (uint64_t*) 0) {
      if(getThreadParent(getContextCL(tempExecContextCL)) == context) {
        println();print(getName(getContextCL(tempExecContextCL)));print((uint64_t*) " dying");println();

        execContexts = removeContextCL(getContextCL(tempExecContextCL), execContexts);
        usedContexts = deleteContext(getContextCL(tempExecContextCL), usedContexts);
      }

      tempExecContextCL = getNextCL(tempExecContextCL);
    }
  }
...
uint64_t* getNextExecutableContext2(uint64_t* context) {
  ...
  if(existsExecutableContext()) {
    //println();print((uint64_t*) "AAA");print((uint64_t*) getName(context));
    nextElement = getNextCL(findElementCL(context,execContexts));

    if (nextElement == (uint64_t*) 0)
      nextElement = execContexts;

    //if (getParent(getContextCL(nextElement)) != MY_CONTEXT)
    //  return getNextExecutableContext2(getContextCL(nextElement));

    if (getState(getContextCL(nextElement)) == STATE_WAITING) {
      handleWaitingState(getContextCL(nextElement));
      return getNextExecutableContext2(getContextCL(nextElement));
    }

    if (getState(getContextCL(nextElement)) == STATE_READY)
      return getContextCL(nextElement);

    if (getState(getContextCL(nextElement)) == STATE_EXITED)
      return getNextExecutableContext2(getContextCL(nextElement));
...
uint64_t* getNextExecutableContext(uint64_t* context) {
  uint64_t* foundElement;
  uint64_t* nextElement;

  if(existsExecutableContext()) {

    foundElement = (uint64_t*) 0;
    foundElement = findElementCL(context, execContexts);
    //print((uint64_t*) "foundEle");printInteger(foundElement);

    if (foundElement == (uint64_t*) 0)
      return (uint64_t*) 0;

    nextElement = getNextCL(foundElement);
    if(nextElement == (uint64_t*) 0)
      nextElement = execContexts;

    //iterate exactly 1 time over list
    while (nextElement != foundElement) {
      ...
      nextElement = getNextCL(nextElement);

      if(nextElement == (uint64_t*) 0)
        nextElement = execContexts;
    }

    //if(getState(getContextCL(foundElement)) == STATE_WAITING)

    if (getState(getContextCL(foundElement)) == STATE_READY)
      if (getParent(getContextCL(foundElement)) == MY_CONTEXT)
        return getContextCL(foundElement);
  }
...
uint64_t handleSystemCalls(uint64_t* context) {
  ...
  if (getException(context) == EXCEPTION_SYSCALL) {
    ...
    if (v0 == SYSCALL_MALLOC)
      ...
      if(writeLockContext == (uint64_t*) 0) {
      ...
      } else if (context == writeLockContext) {
        if (debug_locks) {
          print(selfieName);
          print((uint64_t*) ": locked context ");
          printInteger(context);
          print((uint64_t*) " is writing: ");
        }
        ...
        if(debug_locks) {
          ...
          print((uint64_t*) ": context ");
          printInteger(context);
          print((uint64_t*) "  would like to write but lock is taken");
        }
      }
...
uint64_t multimipster() {
  ...
  toContext = getContextCL(execContexts);

  print((uint64_t*) "multimipster");
  println();
  ...
  while (1) {
    ...
    if (getParent(fromContext) != MY_CONTEXT) {
      ...
      toContext = getNextExecutableContext2(fromContext);

      if (getException(fromContext) == EXCEPTION_PAGEFAULT) {
        // TODO: use this table to unmap and reuse frames
        //mapPage(fromContext, getFaultingPage(fromContext), (uint64_t) palloc());
        mapOrLinkPage(fromContext,getFaultingPage(fromContext));
			} else if (handleSystemCalls(fromContext) == EXIT) {
      ...
      //println();print((uint64_t*) "toContext: ");printInteger((uint64_t)toContext); println();

      if(toContext == (uint64_t*) 0) {
        debug_switch = 0;
        return 0;
      }
...
uint64_t multihypster() {
  ...
  toContext = getContextCL(execContexts);
  print((uint64_t*) "multihypster");
  println();

  while (1) {

    //println();print((uint64_t*) "AAA: before hypster switch");println();
    fromContext = hypster_switch(toContext, timeout);
    //println();print((uint64_t*) "BBB: after hypster switch");println();
     // we are the parent in charge of handling exceptions
...
uint64_t selfie_run(uint64_t machine) {
  ...
  firstContext = createContext(MY_CONTEXT, 0);
  up_loadBinary(firstContext);
  // pass binary name as first argument by replacing memory size
  setArgument(binaryName);
  up_loadArguments(firstContext, numberOfRemainingArguments(), remainingArguments());

  secondContext = createContext(MY_CONTEXT, 0);
  up_loadBinary(secondContext);
  setArgument(binaryName);
  up_loadArguments(secondContext, numberOfRemainingArguments(), remainingArguments());

  thirdContext = createContext(MY_CONTEXT, 0);
  up_loadBinary(thirdContext);
  setArgument(binaryName);
  up_loadArguments(thirdContext, numberOfRemainingArguments(), remainingArguments());

  execContexts = addContextCL(firstContext, execContexts);
  //execContexts = addContextCL(secondContext, execContexts);
  //execContexts = addContextCL(thirdContext, execContexts);
  ...
  if (machine == MIPSTER)
    exitCode = multimipster();
	else if (machine == DOUBLEMIPSTER) {
    execContexts = addContextCL(secondContext, execContexts);
    //execContexts = addContextCL(thirdContext, execContexts);
    exitCode = multimipster();
  }
	else if (machine == DOUBLEHYPSTER) {
    execContexts = addContextCL(secondContext, execContexts);
    //execContexts = addContextCL(thirdContext, execContexts);
    if (isBootLevelZero())
  ...
  print((uint64_t*) " all contexts with exit code ");
  ...
  if (numberOfRemainingArguments() == 0)
    ...
    while (numberOfRemainingArguments() > 0) {
      ...
      if (stringCompare(option, (uint64_t*) "-c"))
        ...
        return selfie_run(DOUBLEHYPSTER);
