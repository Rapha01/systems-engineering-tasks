...
uint64_t* allocateContext(uint64_t* parent, uint64_t* vctxt, uint64_t* in) {
  ...
  if (freeContexts == (uint64_t*) 0)
    context = smalloc(7 * SIZEOFUINT64STAR + 13 * SIZEOFUINT64);
  else {
...
void printOverviewCL(uint64_t* cl) {
  ...
  print((uint64_t*) "# | contextPointer | state | pid | name" ); println();
  ...
  while (tempElement != (uint64_t*) 0) {
    ...
    print((uint64_t*) "  " ); printInteger((uint64_t) getContextCL(tempElement));
  ...
  while (1) {
    ...
    if (getParent(fromContext) != MY_CONTEXT) {
      ...
      if (getException(fromContext) == EXCEPTION_PAGEFAULT) {
        ...
        setState(fromContext, STATE_EXITED);
      ...
      //println();print((uint64_t*) "toContext: ");print((uint64_t*) getName(toContext)); println();
