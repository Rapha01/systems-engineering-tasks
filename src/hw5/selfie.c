...
void addExecContext(uint64_t* context);
void removeExecContext(uint64_t* context);
uint64_t* nextRunnableContext(uint64_t* context);
uint64_t* findExecContext(uint64_t* context);
...
uint64_t* getNextExecContext(uint64_t* execContext)    { return (uint64_t*) *execContext; }
uint64_t* getExecContext(uint64_t* execContext)          { return (uint64_t*) *(execContext + 1); }

void setNextExecContext(uint64_t* execContext, uint64_t* next)     { *execContext        = (uint64_t) next; }
void setExecContext(uint64_t* execContext, uint64_t* context)      { *(execContext + 1)  = (uint64_t) context; }
...
void implementFork(uint64_t* context) {
  ...
  *(getRegs(newContext) + REG_V0) = 0;
  maxpid = maxpid + 1;
  *(getRegs(context) + REG_V0)  = maxpid;

  addExecContext(newContext);

  debug_forked = 1;
  if (debug_fork) {
    println();
    print(selfieName);
    print((uint64_t*) ": Fork on context ");print((uint64_t*) getName(context)); println();
...
void printContext(uint64_t* context) {
  ...
  while (r < NUMBEROFREGISTERS) {
    print((uint64_t*) "  " ); print(*(REGISTERS + r));
    print((uint64_t*) "  " ); printInteger(*(regs + r));println();

    r = r + 1;
  }

  //println();print((uint64_t*) "PAGES" ); println();
  //page = 0;
  //pageTable = getPT(context);
  //print((uint64_t*) "mappedpages:      " ); printInteger(page); println();

  println();
}

void addExecContext(uint64_t* context) {
  uint64_t* newExecContext;
  newExecContext = malloc(12);
  setNextExecContext(newExecContext, execContexts);
  setExecContext(newExecContext, context);
  execContexts = newExecContext;
}

void removeExecContext(uint64_t* context) {
  uint64_t* tempExecContext;
  uint64_t* previousExecContext;
  uint64_t* nextExecContext;

  previousExecContext = (uint64_t*) 0;
  tempExecContext = execContexts;
  nextExecContext = (uint64_t*) 0;

  while(tempExecContext != (uint64_t*) 0) {
    nextExecContext = getNextExecContext(tempExecContext);

    if (getExecContext(tempExecContext) == context) {
      if(previousExecContext == (uint64_t*) 0)
        execContexts = nextExecContext;
      else if (nextExecContext  == (uint64_t*) 0)
        setNextExecContext(previousExecContext, (uint64_t*) 0);
    }
    previousExecContext = tempExecContext;
    tempExecContext = getNextExecContext(tempExecContext);
  }
}
...
uint64_t* nextRunnableContext(uint64_t* context) {
  uint64_t* foundExecContext;
  uint64_t* nextExecContext;

  foundExecContext = findExecContext(context);

  if (foundExecContext != (uint64_t*) 0) {
    nextExecContext = getNextExecContext(foundExecContext);

    if(nextExecContext == (uint64_t*) 0)
      nextExecContext = execContexts;

    return getExecContext(nextExecContext);
  }
...
uint64_t multimipster() {
  ...
  toContext = getExecContext(execContexts);
  ...
  while (1) {
    ...
    if (getParent(fromContext) != MY_CONTEXT) {
      ...
      if (getException(fromContext) == EXCEPTION_PAGEFAULT) {
        ...
        fromContext = nextRunnableContext(fromContext);
        removeExecContext(toContext);

        if(execContexts == (uint64_t*) 0) {
					debug_switch = 0;
					return getExitCode(fromContext);
				}
			}
      ...
      setException(fromContext, EXCEPTION_NOEXCEPTION);
			...
			toContext = nextRunnableContext(fromContext);
    }
  }
  debug_switch = 0;
}
...
uint64_t selfie_run(uint64_t machine) {
  ...
  addExecContext(firstContext);
  //addExecContext(secondContext);
  //addExecContext(thirdContext);
